...
namespace ITI.PixLogic.WinApp
{
	...
	public partial class CapitalizedView : Form
	{
		...
		{    
            ...
            for( int k = 7; k < CapitalizedDataListView.Columns.Count - 1; k++ )
            {
		...
		{
            AddCapitalizedView editForm = new AddCapitalizedView( this );


            var capToEdit = (CapitalizedItemModel)CapitalizedDataListView.SelectedObject;
            var a = editForm.Controls;
            editForm.Reference = capToEdit.Reference;
            editForm.EAN13 = capToEdit.EAN13;
            editForm.Description = capToEdit.Description;
            //editForm.SubCategory = capToEdit.SubCategory;
            //editForm.Reservable = capToEdit.Reservable;
            editForm.Show( this.CapitalizedDataListView );
            editForm.Refresh();
        ...
        private void CapitalizedDataListView_CellEditFinishing( object sender, BrightIdeasSoftware.CellEditEventArgs e )
        {
            if ( CapitalizedDataListView.SelectedItems.Count == 0 )
			{
				MessageBox.Show( "Veuillez selectionner une ligne a supprimer" );
			...
			{
                var toBeUpdated =(CapitalizedItemModel)CapitalizedDataListView.SelectedObject;
                   
                var capitalizedData =_itemsEntity.Items.First( c => c.Id == toBeUpdated.Id );
                _itemsEntity.Items.Remove( capitalizedData );
                try
                {
                    _itemsEntity.SaveChanges();
